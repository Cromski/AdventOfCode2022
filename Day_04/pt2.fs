...
let wing (arr: string list) =
    let rec aux acc (arr: string list) =
        ...
        | x :: xs ->
            ...
            if 
                ((intList[0] < intList[2] && intList[0] < intList[3]) && (intList[1] < intList[2] && intList[1] < intList[3])) ||
                ((intList[0] > intList[2] && intList[0] > intList[3]) && (intList[1] > intList[2] && intList[1] > intList[3])) // not overlapping
            then aux acc xs
            else aux (acc+1) xs
    aux 0 arr
